...
export default function StyleApprovalsPage() {
  ...
  const loadPending = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const pending = await fetchPendingStyles();
      setStyles(pending);
      setSelectedId((current) => {
        if (current && pending.some((s) => s.id === current)) return current;
        return pending[0]?.id ?? null;
      });
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to load pending styles");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  }, [authState, loadPending]);

  async function handleStatus(styleId: string, status: "approved" | "rejected") {
    setWorkingId(styleId);
    setError(null);
    try {
      await updateStyleStatus(styleId, status);
      setStyles((current) => {
        const next = current.filter((style) => style.id !== styleId);
        if (selectedId === styleId) setSelectedId(next[0]?.id ?? null);
        return next;
      });
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to update style");
    } finally {
      setWorkingId(null);
    }
  }
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        <header className="h-[64px] border-b border-slate-200 bg-white flex items-center justify-between px-5 shrink-0">
          <div className="flex items-center gap-3">
            <div className="size-8 rounded-md bg-indigo-50 text-indigo-700 grid place-items-center">
              <ShieldCheck className="size-4" />
            </div>
            <div>
              <h1 className="text-base font-bold leading-tight">Style Approvals</h1>
        ...
        {error && (
          <div className="mx-5 mt-3 rounded-md border border-rose-100 bg-rose-50 px-3 py-2 text-sm text-rose-700 shrink-0">
            {error}
          </div>
        )}
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className="text-[9px] font-bold uppercase tracking-wider rounded-full bg-amber-100 text-amber-700 px-1.5 py-0.5 shrink-0">
                          {style.kind}
                        </span>
                      </div>
    ...
    <div className="flex flex-col min-h-0 flex-1">
      <div className="h-11 px-4 border-b border-slate-100 flex items-center justify-between shrink-0">
        <div className="min-w-0">
          <div className="text-[13px] font-semibold text-slate-800 truncate">{style.name}</div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Button
            ...
            onClick={onReject}
            disabled={working}
            className="h-8 text-xs text-rose-700 border-rose-200 hover:bg-rose-50 hover:text-rose-800"
          >
            <X className="size-3" />
            Reject
          </Button>
      ...
      <ScrollArea className="flex-1 min-h-0">
        <div className="p-5 space-y-4">
          <div className="rounded-md overflow-hidden bg-slate-950 border border-slate-800 aspect-video grid place-items-center">
            {style.kind === "code" ? (
              compileError ? (
                <div className="text-rose-300 text-xs font-mono p-4 max-w-full overflow-auto">
              ...
              <div className="w-full p-6">
                <CommunityStylePreview style={style} action="none" />
              </div>
            )}
          </div>

          <div className="grid grid-cols-2 gap-3 text-[12px]">
            <Meta label="Status" value={style.status} />
            <Meta label="Kind" value={style.kind} />
            <Meta label="Base style" value={style.baseStyle} />
            <Meta label="Submitted" value={new Date(style.createdAt).toLocaleString()} />
          </div>
          ...
          {style.kind === "code" && style.code && (
            <div className="space-y-1">
              <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
                Source
              </div>
              <pre className="rounded-md bg-[#1e1e2e] text-[#cdd6f4] text-[11.5px] font-mono p-3 max-h-72 overflow-auto whitespace-pre-wrap break-words">
                {style.code}
              </pre>
            </div>
          )}

          {style.kind === "code" && !Component && !compileError && (
...
function Meta({label, value}: {label: string; value: string}) {
  return (
    <div className="rounded-md border border-slate-100 bg-slate-50 px-3 py-2">
      ...
      <div className="text-[12.5px] text-slate-700 font-medium truncate">{value}</div>
    </div>
  );
}
